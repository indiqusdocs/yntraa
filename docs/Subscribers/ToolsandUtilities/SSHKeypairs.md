...
To generate new SSH key pairs, click the **Generate a new key pair** button. ![generatesshkey](img/generatesshkey.png)
...
![uploadsshkey](img/uploadsshkey.png)
...
1. While creating a [new Linux Instance](/docs/Subscribers/Compute/LinuxInstances/CreatingLinuxInstances), in the provisioning details form, choose any of the SSH options from the **Use SSH key pair** option under **Choose an Authentication method**. This will let you:
    ...
    3. Select an existing SSH key from your Yntraa Cloud Console account. ![choosesshkey](img/choosesshkey.png)
...
If a SSH public key is deleted from the Yntraa Cloud Console account, you'll no longer be able to connect to the Linux Instance(s) associated with the key pair, however you can use a password to connect to the Instance. ![deletessh](img/deletessh.png)
