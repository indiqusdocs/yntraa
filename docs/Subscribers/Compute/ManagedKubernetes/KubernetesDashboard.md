...
- **Access Control:** Access to the dashboard is subject to Role-Based Access Control (RBAC) policies. Users need appropriate permissions to view and modify resources.
...
Details of services, pods, namespaces, controllers etc. for a Kubernetes cluster are available on the Kubernetes dashboard UI. 

To access the dashboard, the `kubeconfig` manifest must be downloaded and used.
...
Once done, a local proxy must be run using the following command:
...
**Obtaining Token for Dashboard Login**
