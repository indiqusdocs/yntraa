...
Configure an SSH Key or Login Credentials to securely access the compute instance after deployment. SSH keys provide a secure authentication method for connecting to the instance, while login credentials such as a username and password can also be used for access and management. For more information on configuring an SSH key of login credentials, [click here](/docs/Subscribers/ToolsandUtilities/ManagingSSHKeysandKeyPairsforLinuxInstances).
